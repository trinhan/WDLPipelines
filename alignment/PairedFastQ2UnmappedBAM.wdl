...
## 
## MODIFICATIONS:
## Change disk space to 4x (fastq1+fastq2). Note that approximately 8x the size of a fastq file is required for this to work
##
...
task PairedFastQsToUnmappedBAM {
  input {
    ...
    Int disk_space_gb = ceil((size(fastq_1, "GB") + size(fastq_2, "GB")) * 4 ) + addtional_disk_space_gb
  command {
